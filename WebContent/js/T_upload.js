(function ($) {
	$.Tupload = {
		...
		createHtml:function(defaults){
			...
			html += 	'<ul>';
				...
				for(var i=0;i<fileNum;i++){
					...
					html += 	'<img id="img_src'+i+'" class="upload_image" src="img/imgadd.png"/>';
					html += '</div>';
		...
		imgLoad:function(i,file){
			...
			$(r).load(function(){
				while(true){
					if($("#img_src"+i).attr("src") != "img/imgadd.png"){
						i++;
					}else{
						break;
					}
...
//		},
		onDelete:function(i){
			...
			arrFile[i] = '';
			$("#img_src"+i).attr("src","img/imgadd.png");
			$("#uploading-tip"+i).hide();
			$.Tupload.setPhotoImg();
		}
	}
...
$("#fileImage").live('change',function(){
	...
	if(num < $.Tupload.fileNum+1){
		...
		for(var i=0;i<num;i++){
			for(var j=0;j<num;j++){
				if($("#img_src"+j).attr("src") == "img/imgadd.png"){
